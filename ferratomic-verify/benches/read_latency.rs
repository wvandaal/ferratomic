...
/// INV-FERR-027: benchmark EAVT range scan over a 10K-datom store.
///
...
fn bench_eavt_range_scan_10k(c: &mut Criterion) {
    let mut group = c.benchmark_group("inv_ferr_027_eavt_range_scan");
    ...
    group.bench_function(BenchmarkId::new("eavt_range_scan_10k", datom_count), |b| {
        b.iter(|| {
            let indexes = store
                .indexes()
            ...
            assert!(
                scanned > 0,
                "INV-FERR-027: EAVT range scan must find at least one datom in range",
            );
            black_box(scanned);
        });
    });
...
criterion_group!(benches, bench_read_latency, bench_eavt_range_scan_10k);
