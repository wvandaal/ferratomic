...
mod mmap_impl {
    ...
    impl MappedStore {
        ...
        pub(crate) fn datom_count(&self) -> u64 {
            self.datom_count
        }
        ...
        pub(crate) fn promote_to_positional(&self) -> Result<PositionalStore, FerraError> {
            ...
            Ok(PositionalStore::from_sorted_with_live(datoms, live_bits))
        }
    }
    ...
    pub(crate) fn mmap_cold_start(path: &std::path::Path) -> Result<MappedStore, FerraError> {
    ...
    pub(crate) fn serialize_mmap_checkpoint(
        datoms: &[Datom],
...
#[cfg(feature = "mmap")]
pub(crate) use mmap_impl::{mmap_cold_start, serialize_mmap_checkpoint, MappedStore, MmapPayload};
