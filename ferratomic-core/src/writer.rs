...
use ferratom::{AgentId, Attribute, Datom, EntityId, Op, Schema, TxId, Value, ValueType};

// ---------------------------------------------------------------------------
// Typestate markers
// ---------------------------------------------------------------------------

/// Marker: transaction is being assembled. Datoms can be added.
#[derive(Debug)]
pub struct Building;

/// Marker: transaction has been validated and sealed. Read-only.
#[derive(Debug)]
...
pub enum TxValidationError {
    ...
    /// The value type does not match the attribute's declared `ValueType`.
    SchemaViolation {
    ...
    /// Cardinality violation (reserved for Phase 4b).
...
mod tests {
    ...
    fn test_transaction_assert_datom_accumulates() {
        ...
        let tx = Transaction::new(agent)
            .assert_datom(entity, Attribute::from("user/name"), Value::String("Alice".into()))
    ...
    fn test_inv_ferr_009_commit_valid() {
        ...
        let tx = Transaction::new(agent)
            .assert_datom(entity, Attribute::from("user/name"), Value::String("Bob".into()));
    ...
    fn test_inv_ferr_009_commit_wrong_type() {
        ...
        let tx = Transaction::new(agent)
            .assert_datom(entity, Attribute::from("user/name"), Value::Long(42));
    ...
    fn test_datoms_have_placeholder_tx_id() {
        ...
        let committed = Transaction::new(agent)
            .assert_datom(entity, Attribute::from("user/name"), Value::String("x".into()))
            .commit_unchecked();
    ...
    fn test_value_matches_type_all_variants() {
        ...
        let pairs: Vec<(Value, ValueType)> = vec![
            ...
            (Value::Bytes(Arc::from(vec![0u8])), ValueType::Bytes),
            (Value::Ref(EntityId::from_bytes([0; 32])), ValueType::Ref),
            (Value::BigInt(0), ValueType::BigInt),
            (Value::BigDec(0), ValueType::BigDec),
        ];
    ...
    fn test_value_matches_type_rejects_mismatches() {
        ...
        assert!(
            !value_matches_type(&Value::String(Arc::from("s")), &ValueType::Long),
            "String should not match Long"
        );
        // Long value should NOT match String type.
        assert!(
            !value_matches_type(&Value::Long(42), &ValueType::String),
