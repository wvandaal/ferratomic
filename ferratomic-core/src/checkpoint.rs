...
pub fn write_checkpoint(store: &Store, path: &Path) -> Result<(), FerraError> {
    ...
    // Build the file content: header + payload.
    let file_size = HEADER_SIZE + payload_bytes.len() + HASH_SIZE;
...
pub fn load_checkpoint(path: &Path) -> Result<Store, FerraError> {
    ...
    let payload: CheckpointPayload = serde_json::from_slice(payload_bytes)
        .map_err(|e| FerraError::CheckpointCorrupted {
            expected: "valid JSON payload".to_string(),
            actual: e.to_string(),
        })?;

    Ok(Store::from_checkpoint(epoch, payload.genesis_agent, payload.schema, payload.datoms))
}
...
fn parse_header(content: &[u8]) -> Result<(u64, &[u8]), FerraError> {
    ...
    let magic: [u8; 4] = content[0..4].try_into().map_err(|_| {
    ...
    let version = u16::from_le_bytes(
        content[4..6].try_into().map_err(|_| FerraError::CheckpointCorrupted {
    ...
    let epoch = u64::from_le_bytes(
        content[6..14].try_into().map_err(|_| FerraError::CheckpointCorrupted {
    ...
    let payload_len = u32::from_le_bytes(
        content[14..18].try_into().map_err(|_| FerraError::CheckpointCorrupted {
...
mod tests {
    ...
    fn test_inv_ferr_013_roundtrip_with_datoms() {
        ...
        assert!(loaded.indexes().verify_bijection(),
            "INV-FERR-005: all indexes must have same cardinality after load");
        assert_eq!(loaded.indexes().len(), loaded.len(),
